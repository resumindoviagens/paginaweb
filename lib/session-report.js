...
function isExplicitContactReason(value) {
  return cleanText(value, 160).toLowerCase().includes("whatsapp");
}

export function shouldNotifyTeam(messages = [], reason = "") {
  const hasVisitorQuestion = messages.some(message =>
    (message.sender_type === "visitor" || message.role === "user") &&
    cleanText(message.content, 1200).length > 0
  );
  return hasVisitorQuestion || isExplicitContactReason(reason);
}
...
export async function sendSessionReport({ db, session, messages = [], reason, page, endedAt }) {
  ...
  if (session.report_sent_at) {
    const skipped = String(session.report_message_id || "").startsWith("skipped:");
    return {
      ok: true,
      alreadySent: !skipped,
      alreadyHandled: true,
      ...
      noQuestions: skipped,
      emailSent: !skipped,
      messageId: session.report_message_id || null
    };
  }
  ...
  if (!shouldNotifyTeam(normalizedMessages, reason)) {
    const skipReason = "sem pergunta — notificação não enviada";
    const { error: skipError } = await db.from("chat_sessions").update({
      status: "closed",
      ended_at: endedAtRaw,
      last_activity: endedAtRaw,
      report_sent_at: endedAtRaw,
      report_message_id: "skipped:no-question",
      report_reason: skipReason
    }).eq("id", session.id);
    if (skipError) console.error("Session no-question close error", skipError);
    return {
      ok: true,
      emailSent: false,
      skipped: true,
